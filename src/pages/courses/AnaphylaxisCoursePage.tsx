...
export function AnaphylaxisCoursePage() {
  ...
  return (
    <Layout>
      ...
      {/* Header */}
      <div className="bg-gradient-to-br from-[#0d9488] to-[#0f766e] text-white rounded-xl px-5 pt-5 pb-6 mb-6">
        <div className="flex items-start justify-between gap-4 mb-3">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-widest text-teal-200 mb-1">CPD — Safety Academy</p>
            <h1 className="text-2xl font-black leading-tight" style={{ fontFamily: 'Montserrat, sans-serif' }}>{course.title}</h1>
            <p className="text-teal-100 text-sm mt-1">{course.subtitle}</p>
          </div>
      ...
      {/* Benedict's Law notice */}
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4 flex items-start gap-3">
        <span className="text-xl shrink-0">⚖️</span>
        <div>
          <p className="text-sm font-bold text-amber-800" style={{ fontFamily: 'Montserrat, sans-serif' }}>Benedict's Law — September 2026 Requirement</p>
          <p className="text-xs text-amber-700 mt-0.5">All staff working at gymnastics clubs must have received anaphylaxis training from September 2026. This course provides the required evidence of training.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 pb-10">
        ...
        {certificate ? (
          <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-center gap-3 mb-1">
            <Award size={28} className="text-teal-600 shrink-0" />
            <div className="flex-1">
              <p className="font-black text-teal-800 text-sm" style={{ fontFamily: 'Montserrat, sans-serif' }}>Certificate Earned</p>
              <p className="text-xs text-gray-500">
                Completed {new Date(certificate.completed_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
              </p>
              <p className="text-xs text-teal-700 font-semibold mt-0.5">{course.certificateTitle}</p>
            </div>
            <CertificateDownload
        ...
        <EnrollmentGate courseId={ANAPHYLAXIS_COURSE.id} courseTitle={ANAPHYLAXIS_COURSE.title}>
        ...
        {loading ? (
          ...
          course.modules.map((mod, index) => {
            ...
            return (
              <button
                key={mod.id}
                onClick={() => unlocked && navigate(`/courses/anaphylaxis/${mod.id}`)}
                disabled={!unlocked}
              ...
              >
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0 bg-gradient-to-br from-[#0d9488] to-[#0f766e]">
        ...
        <p className="text-xs text-gray-400 text-center pt-2">
          Complete each module in order. A certificate is awarded on completion and saved to your profile as evidence of training.
        </p>
      </div>
    </Layout>
  )
