...
export function AreaLeadModulePage() {
  const { moduleId } = useParams<{ moduleId: string }>()
  const { profile } = useAuth()
  const mod = AREA_LEAD_COURSE.modules.find(m => m.id === moduleId)
  ...
  if (!mod) return <Navigate to="/courses/area-lead" replace />

  const moduleIndex = AREA_LEAD_COURSE.modules.findIndex(m => m.id === moduleId)
  const prevModule = moduleIndex > 0 ? AREA_LEAD_COURSE.modules[moduleIndex - 1] : null
  const nextModule = moduleIndex < AREA_LEAD_COURSE.modules.length - 1 ? AREA_LEAD_COURSE.modules[moduleIndex + 1] : null
  const isLastModule = moduleIndex === AREA_LEAD_COURSE.modules.length - 1
    ...
    if (passed) {
      ...
      if (isLastModule) {
        const { data: existing } = await supabase
          ...
          .eq('course_id', AREA_LEAD_COURSE.id)
          .maybeSingle()
  ...
  return (
    <Layout>
      <div className="mb-2">
        <Link to="/courses/area-lead" className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 mb-4">
          <ArrowLeft size={14} />
          Back to course
        </Link>
      </div>
      ...
      <div className={`bg-gradient-to-br ${mod.gradient} rounded-xl p-6 mb-6 flex items-center gap-4`}>
        ...
        <div>
          <div className="text-xs font-bold text-white/60 uppercase tracking-wide mb-0.5">
            Module {mod.number} of {AREA_LEAD_COURSE.modules.length}
          </div>
              ...
              return (
                <div key={qi} className="bg-white rounded-xl border border-gray-200 p-5">
                  <div className="flex items-start gap-3 mb-4">
                    <span
                      className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 text-white bg-violet-700"
                      style={{ fontFamily: 'Montserrat, sans-serif' }}
                    >
                      {qi + 1}
                    </span>
      ...
      <div className="mt-8 flex justify-between gap-4">
        {prevModule ? (
          <Link
            to={`/courses/area-lead/${prevModule.id}`}
        ...
        {nextModule ? (
          <Link
            to={`/courses/area-lead/${nextModule.id}`}
