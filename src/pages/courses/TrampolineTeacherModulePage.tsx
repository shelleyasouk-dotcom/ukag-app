...
import { TRAMPOLINE_TEACHER_COURSE } from '../../data/trampolineTeacherCourse'
...
export function TrampolineTeacherModulePage() {
  const { moduleId } = useParams<{ moduleId: string }>()
  const { profile } = useAuth()
  const mod = TRAMPOLINE_TEACHER_COURSE.modules.find(m => m.id === moduleId)
  ...
  if (!mod) return <Navigate to="/courses/trampoline-teacher" replace />

  const moduleIndex = TRAMPOLINE_TEACHER_COURSE.modules.findIndex(m => m.id === moduleId)
  const prevModule = moduleIndex > 0 ? TRAMPOLINE_TEACHER_COURSE.modules[moduleIndex - 1] : null
  const nextModule = moduleIndex < TRAMPOLINE_TEACHER_COURSE.modules.length - 1 ? TRAMPOLINE_TEACHER_COURSE.modules[moduleIndex + 1] : null
  const isLastModule = moduleIndex === TRAMPOLINE_TEACHER_COURSE.modules.length - 1
  ...
  async function submitQuiz() {
    ...
    const score = mod.quiz.reduce((acc, q, i) => acc + (answers[i] === q.answer ? 1 : 0), 0)
    ...
    if (passed) {
      ...
      await supabase.from('course_progress').upsert({
        user_id: profile.id,
        course_id: TRAMPOLINE_TEACHER_COURSE.id,
        module_id: mod.id,
      ...
      if (isLastModule) {
        const { data: existing } = await supabase
          ...
          .eq('course_id', TRAMPOLINE_TEACHER_COURSE.id)
          .maybeSingle()
  ...
  return (
    <Layout>
      <div className="mb-2">
        <Link to="/courses/trampoline-teacher" className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 mb-4">
          <ArrowLeft size={14} />
          Back to course
        </Link>
      </div>
      ...
      {/* Module header */}
      <div className="bg-gradient-to-br from-cyan-700 to-teal-800 rounded-xl p-6 mb-6 flex items-center gap-4">
        ...
        <div>
          <div className="text-xs font-bold text-white/60 uppercase tracking-wide mb-0.5">
            Module {moduleIndex + 1} of {TRAMPOLINE_TEACHER_COURSE.modules.length}
          </div>
              ...
              <ul className="space-y-2">
                {section.content.map((item, ci) => (
                  <li key={ci} className="flex items-start gap-2 text-sm text-gray-700">
                    <span className="w-1.5 h-1.5 rounded-full flex-shrink-0 mt-2" style={{ backgroundColor: '#0e7490' }} />
                    {item}
                  </li>
                ))}
              </ul>
        ...
        <div>
          {quizState === 'passed' && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center mb-6">
              ...
              {isLastModule && (
                <p className="text-xs text-green-600 mt-1">Your UKAG Trampolining Teacher Certificate has been awarded and saved to your profile.</p>
              )}
            </div>
          )}
          ...
          {quizState === 'failed' && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center mb-6">
              ...
              <p className="text-sm text-red-700 mb-4">
                You scored {score}/{mod.quiz.length}. You need {PASS_THRESHOLD} to pass.
              </p>
              <button
                onClick={retryQuiz}
              ...
              return (
                <div key={qi} className="bg-white rounded-xl border border-gray-200 p-5">
                  <div className="flex items-start gap-3 mb-4">
                    <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 text-white" style={{ backgroundColor: '#0e7490', fontFamily: 'Montserrat, sans-serif' }}>
                      {qi + 1}
                    </span>
                  ...
                  <div className="space-y-2">
                    {q.options.map((opt, ai) => {
                      ...
                      if (chosen === ai && !isRevealed) style = 'border-cyan-500 bg-cyan-50 text-cyan-800'
                      if (isRevealed && ai === q.answer) style = 'border-green-400 bg-green-50 text-green-800'
                      else if (isRevealed && chosen === ai && ai !== q.answer) style = 'border-red-400 bg-red-50 text-red-800'
      ...
      <div className="mt-8 flex justify-between gap-4">
        {prevModule ? (
          <Link
            to={`/courses/trampoline-teacher/${prevModule.id}`}
        ...
        {nextModule ? (
          <Link
            to={`/courses/trampoline-teacher/${nextModule.id}`}
