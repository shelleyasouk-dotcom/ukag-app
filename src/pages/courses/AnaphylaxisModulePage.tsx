...
export function AnaphylaxisModulePage() {
  const { moduleId } = useParams<{ moduleId: string }>()
  const { profile } = useAuth()
  const mod = ANAPHYLAXIS_COURSE.modules.find(m => m.id === moduleId)
  ...
  if (!mod) return <Navigate to="/courses/anaphylaxis" replace />

  const moduleIndex = ANAPHYLAXIS_COURSE.modules.findIndex(m => m.id === moduleId)
  const prevModule = moduleIndex > 0 ? ANAPHYLAXIS_COURSE.modules[moduleIndex - 1] : null
  const nextModule = moduleIndex < ANAPHYLAXIS_COURSE.modules.length - 1 ? ANAPHYLAXIS_COURSE.modules[moduleIndex + 1] : null
  const isLastModule = moduleIndex === ANAPHYLAXIS_COURSE.modules.length - 1
  ...
  async function submitQuiz() {
    ...
    if (passed) {
      ...
      await supabase.from('course_progress').upsert({
        user_id: profile.id,
        course_id: ANAPHYLAXIS_COURSE.id,
        module_id: mod.id,
      ...
      if (isLastModule) {
        const { data: existing } = await supabase
          ...
          .eq('course_id', ANAPHYLAXIS_COURSE.id)
          .maybeSingle()
  ...
  return (
    <Layout>
      <div className="mb-2">
        <Link to="/courses/anaphylaxis" className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 mb-4">
          <ArrowLeft size={14} />
          Back to course
        </Link>
      </div>
      ...
      {/* Module header */}
      <div className="bg-gradient-to-br from-[#0d9488] to-[#0f766e] rounded-xl p-6 mb-6 flex items-center gap-4">
        <span className="text-4xl">{mod.emoji}</span>
        <div>
          <div className="text-xs font-bold text-white/60 uppercase tracking-wide mb-0.5">
            Module {mod.number} of {ANAPHYLAXIS_COURSE.modules.length}
          </div>
                ...
                <ul className="space-y-2">
                  {section.bullets.map((b, bi) => (
                    <li key={bi} className="flex items-start gap-2 text-sm text-gray-700">
                      <span className="w-1.5 h-1.5 rounded-full flex-shrink-0 mt-2 bg-teal-600" />
                      {b}
                    </li>
                  ))}
                </ul>
        ...
        <div>
          {quizState === 'passed' && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center mb-6">
              ...
              {isLastModule && (
                <p className="text-xs text-green-600 mt-1">Your Anaphylaxis Awareness Certificate has been awarded and saved to your profile as evidence of training.</p>
              )}
            </div>
          )}
              ...
              return (
                <div key={qi} className="bg-white rounded-xl border border-gray-200 p-5">
                  <div className="flex items-start gap-3 mb-4">
                    <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 text-white" style={{ backgroundColor: '#0d9488', fontFamily: 'Montserrat, sans-serif' }}>
                      {qi + 1}
                    </span>
                  ...
                  <div className="space-y-2">
                    {q.options.map((opt, ai) => {
                      ...
                      if (chosen === ai && !isRevealed) style = 'border-teal-500 bg-teal-50 text-teal-800'
      ...
      <div className="mt-8 flex justify-between gap-4">
        {prevModule ? (
          <Link
            to={`/courses/anaphylaxis/${prevModule.id}`}
        ...
        {nextModule ? (
          <Link
            to={`/courses/anaphylaxis/${nextModule.id}`}
