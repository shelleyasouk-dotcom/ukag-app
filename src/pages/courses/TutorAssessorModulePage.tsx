...
export function TutorAssessorModulePage() {
  const { moduleId } = useParams<{ moduleId: string }>()
  const { profile } = useAuth()
  const mod = TUTOR_ASSESSOR_COURSE.modules.find(m => m.id === moduleId)
  ...
  if (!mod) return <Navigate to="/courses/tutor-assessor" replace />

  const moduleIndex = TUTOR_ASSESSOR_COURSE.modules.findIndex(m => m.id === moduleId)
  const prevModule = moduleIndex > 0 ? TUTOR_ASSESSOR_COURSE.modules[moduleIndex - 1] : null
  const nextModule = moduleIndex < TUTOR_ASSESSOR_COURSE.modules.length - 1 ? TUTOR_ASSESSOR_COURSE.modules[moduleIndex + 1] : null
  const isLastModule = moduleIndex === TUTOR_ASSESSOR_COURSE.modules.length - 1
  ...
  async function submitQuiz() {
    ...
    if (passed) {
      ...
      await supabase.from('course_progress').upsert({
        user_id: profile.id,
        course_id: TUTOR_ASSESSOR_COURSE.id,
        module_id: mod.id,
      ...
      if (isLastModule) {
        const { data: existing } = await supabase
          ...
          .eq('course_id', TUTOR_ASSESSOR_COURSE.id)
          .maybeSingle()
  ...
  return (
    <Layout>
      <div className="mb-2">
        <Link to="/courses/tutor-assessor" className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 mb-4">
          <ArrowLeft size={14} />
          Back to course
        </Link>
      </div>
      ...
      <div className={`bg-gradient-to-br ${mod.gradient} rounded-xl p-6 mb-6 flex items-center gap-4`}>
        ...
        <div>
          <div className="text-xs font-bold text-white/60 uppercase tracking-wide mb-0.5">
            Module {mod.number} of {TUTOR_ASSESSOR_COURSE.modules.length}
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-end">
            <button
              ...
              style={{ backgroundColor: '#6d28d9', fontFamily: 'Montserrat, sans-serif' }}
            >
              Take the Quiz
              <ArrowRight size={16} />
            </button>
                ...
                <div key={qi} className="bg-white rounded-xl border border-gray-200 p-5">
                  ...
                  <div className="space-y-2">
                    {q.options.map((opt, ai) => {
                      ...
                      if (chosen === ai && !isRevealed) style = 'border-[#6d28d9] bg-[#6d28d9]/10 text-[#6d28d9]'
      ...
      <div className="mt-8 flex justify-between gap-4">
        {prevModule ? (
          <Link
            to={`/courses/tutor-assessor/${prevModule.id}`}
        ...
        {nextModule ? (
          <Link
            to={`/courses/tutor-assessor/${nextModule.id}`}
