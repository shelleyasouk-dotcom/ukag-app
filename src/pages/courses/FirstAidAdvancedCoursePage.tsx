      ...
      {certificate && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 flex items-center gap-3">
          ...
          <CertificateDownload
            ...
            courseTitle={FIRST_AID_ADVANCED_COURSE.certificateTitle}
            completedAt={certificate.completed_at}
            certificateId={certificate.id}
            courseId={FIRST_AID_ADVANCED_COURSE.id}
            userId={profile?.id}
          />
        </div>
      )}

      <EnrollmentGate courseId={FIRST_AID_ADVANCED_COURSE.id} courseTitle={FIRST_AID_ADVANCED_COURSE.title}>
        {/* Module list */}
