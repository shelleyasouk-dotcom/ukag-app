...
export function EqualityInclusionCoursePage() {
  ...
  const course = EQUALITY_INCLUSION_COURSE
  ...
  return (
    <Layout>
      ...
      <div className="bg-gradient-to-br from-rose-600 to-pink-700 text-white rounded-xl px-5 pt-5 pb-6 mb-6">
        ...
        <div className="mt-4">
          <div className="flex justify-between text-xs text-rose-200 mb-1.5">
            <span>{doneCount} of {total} modules complete</span>
            <span>{Math.round((doneCount / total) * 100)}%</span>
          </div>
          <div className="h-2 bg-white/20 rounded-full overflow-hidden">
            <div
              className="h-full bg-rose-200 rounded-full transition-all duration-500"
      ...
      <div className="flex flex-col gap-3 pb-10">
        ...
        {certificate ? (
          <div className="bg-rose-50 border border-rose-200 rounded-2xl p-4 flex items-center gap-3 mb-1">
            <Award size={28} className="text-rose-600 shrink-0" />
            <div className="flex-1">
              <p className="font-black text-rose-800 text-sm" style={{ fontFamily: 'Montserrat, sans-serif' }}>Certificate Earned</p>
              <p className="text-xs text-gray-500">
                Completed {new Date(certificate.completed_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
              </p>
              <p className="text-xs text-rose-700 font-semibold mt-0.5">{course.certificateTitle}</p>
            </div>
            <CertificateDownload
        ...
        <EnrollmentGate courseId={EQUALITY_INCLUSION_COURSE.id} courseTitle={EQUALITY_INCLUSION_COURSE.title}>
            ...
            {loading ? (
              ...
              course.modules.map((mod, index) => {
                ...
                return (
                  <button
                    key={mod.id}
                    onClick={() => unlocked && navigate(`/courses/equality/${mod.id}`)}
                    disabled={!unlocked}
                  ...
                  >
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0 bg-gradient-to-br from-rose-600 to-pink-700">
