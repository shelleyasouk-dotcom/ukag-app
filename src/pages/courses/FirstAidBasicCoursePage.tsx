...
export function FirstAidBasicCoursePage() {
  ...
  const total = FIRST_AID_BASIC_COURSE.modules.length
    ...
    <Layout>
      ...
      <div className="bg-gradient-to-br from-orange-600 to-red-700 rounded-xl p-6 mb-6">
        ...
        <h1 className="text-2xl font-black text-white mb-1" style={{ fontFamily: 'Montserrat, sans-serif' }}>
          {FIRST_AID_BASIC_COURSE.title}
        </h1>
        <p className="text-sm text-white/80 mb-4">{FIRST_AID_BASIC_COURSE.description}</p>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="font-black text-amber-800 text-sm" style={{ fontFamily: 'Montserrat, sans-serif' }}>
              {FIRST_AID_BASIC_COURSE.certificateTitle}
            </div>
      ...
      <EnrollmentGate courseId={FIRST_AID_BASIC_COURSE.id} courseTitle={FIRST_AID_BASIC_COURSE.title}>
        {/* Module list */}
        <div className="space-y-3">
        {FIRST_AID_BASIC_COURSE.modules.map((mod, index) => {
          const isDone = completedModules.has(mod.id)
          const isUnlocked = index === 0 || completedModules.has(FIRST_AID_BASIC_COURSE.modules[index - 1].id)
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {isUnlocked && (
                  <Link
                    to={`/courses/first-aid-basic/${mod.id}`}
                    className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold text-white bg-orange-600 hover:bg-orange-700 transition-colors"
