...
export function JuniorCoachModulePage() {
  const { moduleId } = useParams<{ moduleId: string }>()
  const { profile } = useAuth()
  const mod = JUNIOR_COACH_COURSE.modules.find(m => m.id === moduleId)
  ...
  if (!mod) return <Navigate to="/courses/junior-coach" replace />

  const moduleIndex = JUNIOR_COACH_COURSE.modules.findIndex(m => m.id === moduleId)
  const prevModule = moduleIndex > 0 ? JUNIOR_COACH_COURSE.modules[moduleIndex - 1] : null
  const nextModule = moduleIndex < JUNIOR_COACH_COURSE.modules.length - 1 ? JUNIOR_COACH_COURSE.modules[moduleIndex + 1] : null
  const isLastModule = moduleIndex === JUNIOR_COACH_COURSE.modules.length - 1
  ...
  async function submitQuiz() {
    ...
    if (passed) {
      ...
      await supabase.from('course_progress').upsert({
        user_id: profile.id,
        course_id: JUNIOR_COACH_COURSE.id,
        module_id: mod.id,
      ...
      if (isLastModule) {
        const { data: existing } = await supabase
          ...
          .eq('course_id', JUNIOR_COACH_COURSE.id)
          .maybeSingle()
  ...
  return (
    <Layout>
      <div className="mb-2">
        <Link to="/courses/junior-coach" className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 mb-4">
          <ArrowLeft size={14} />
          Back to course
        </Link>
      </div>
      ...
      <div className={`bg-gradient-to-br ${mod.gradient} rounded-xl p-6 mb-6 flex items-center gap-4`}>
        ...
        <div>
          <div className="text-xs font-bold text-white/60 uppercase tracking-wide mb-0.5">
            Module {mod.number} of {JUNIOR_COACH_COURSE.modules.length}
          </div>
      ...
      <div className="mt-8 flex justify-between gap-4">
        {prevModule ? (
          <Link
            to={`/courses/junior-coach/${prevModule.id}`}
        ...
        {nextModule && quizState === 'passed' ? (
          <Link
            to={`/courses/junior-coach/${nextModule.id}`}
