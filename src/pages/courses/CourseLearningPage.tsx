...
const LESSON_ICONS: Record<string, ReactElement> = {
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          <div className="w-full bg-gray-100 rounded-full h-2.5">
            <div className="bg-ukag-600 h-2.5 rounded-full transition-all duration-500" style={{ width: `${overallProgress}%` }} />
          </div>
        ...
        {weeks.length === 0 ? (
          ...
          <div className="space-y-3">
            {weeks.map((week) => {
              ...
              return (
                <div key={week.id} className={`bg-white border rounded-xl overflow-hidden transition-all ${
                  week.isLocked ? 'border-gray-100 opacity-50' :
                  week.isComplete ? 'border-green-200' : 'border-ukag-200 shadow-sm'
                }`}>
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                          ...
                          week.isComplete ? 'bg-green-100 text-green-600' : 'bg-ukag-100 text-ukag-700'
                        }`}>
                          {week.isLocked ? <Lock size={14} /> : week.isComplete ? <CheckCircle2 size={16} /> : week.week_number}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="font-semibold text-gray-900 text-sm">Week {week.week_number} — {week.title}</div>
                          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                            <span className="text-xs text-gray-400">{week.lessons.length} lesson{week.lessons.length !== 1 ? 's' : ''}</span>
                            {week.assessment && <span className="text-xs text-gray-400">· Assessment</span>}
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {week.isComplete ? (
                          ...
                          <>
                            ...
                            <Link to={`/courses/${id}/learn/week/${week.id}`}
                              ...
                              {week.completedLessons > 0 ? 'Continue' : 'Start'}<ChevronRight size={12} />
                            </Link>
                          </>
                        )}
                      </div>
                    ...
                    {!week.isLocked && week.lessons.length > 0 && (
                      <div className="mt-3">
                        <div className="w-full bg-gray-100 rounded-full h-1.5">
                          <div className={`h-1.5 rounded-full transition-all duration-300 ${week.isComplete ? 'bg-green-500' : 'bg-ukag-500'}`}
