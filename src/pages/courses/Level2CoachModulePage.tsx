...
export function Level2CoachModulePage() {
  const { moduleId } = useParams<{ moduleId: string }>()
  const { profile } = useAuth()
  const mod = LEVEL2_COACH_COURSE.modules.find(m => m.id === moduleId)
  ...
  if (!mod) return <Navigate to="/courses/level-2-lead" replace />

  const moduleIndex = LEVEL2_COACH_COURSE.modules.findIndex(m => m.id === moduleId)
  const prevModule = moduleIndex > 0 ? LEVEL2_COACH_COURSE.modules[moduleIndex - 1] : null
  const nextModule = moduleIndex < LEVEL2_COACH_COURSE.modules.length - 1 ? LEVEL2_COACH_COURSE.modules[moduleIndex + 1] : null
  const isLastModule = moduleIndex === LEVEL2_COACH_COURSE.modules.length - 1
  ...
  async function submitQuiz() {
    ...
    if (passed) {
      ...
      await supabase.from('course_progress').upsert({
        user_id: profile.id,
        course_id: LEVEL2_COACH_COURSE.id,
        module_id: mod.id,
  ...
  return (
    <Layout>
      ...
      <div className={`bg-gradient-to-br ${mod.gradient} rounded-xl p-6 mb-6 flex items-center gap-4`}>
        ...
        <div>
          <div className="text-xs font-bold text-white/60 uppercase tracking-wide mb-0.5">
            Module {mod.number} of {LEVEL2_COACH_COURSE.modules.length}
          </div>
        ...
        <div className="space-y-6">
                ...
                <ul className="space-y-2">
                  {section.bullets.map((b, bi) => (
                    <li key={bi} className="flex items-start gap-2 text-sm text-gray-700">
                      <span className="w-1.5 h-1.5 rounded-full flex-shrink-0 mt-2" style={{ backgroundColor: accentColour }} />
                      {b}
                    </li>
                  ))}
                </ul>
          ...
          <div className="flex justify-end">
            <button
              ...
              style={{ backgroundColor: accentColour, fontFamily: 'Montserrat, sans-serif' }}
            >
              Take the Quiz
              <ArrowRight size={16} />
            </button>
        ...
        <div>
          {quizState === 'passed' && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center mb-6">
              ...
              {isLastModule && (
                <>
                  <p className="text-xs text-green-600 mt-1 mb-3">Now complete your practical portfolio to continue.</p>
                  <Link
                    to="/courses/level-2-lead/practical"
                    className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold text-white"
                    style={{ backgroundColor: accentColour, fontFamily: 'Montserrat, sans-serif' }}
                  >
                    Proceed to Practical Portfolio →
                  </Link>
                </>
              )}
            </div>
                ...
                <div key={qi} className="bg-white rounded-xl border border-gray-200 p-5">
                  ...
                  <div className="space-y-2">
                    {q.options.map((opt, ai) => {
                      ...
                      if (chosen === ai && !isRevealed) style = 'border-[#1e52a4] bg-[#1e52a4]/10 text-[#1e52a4]'
      ...
      <div className="mt-8 flex justify-between gap-4">
        {prevModule ? (
          <Link
            to={`/courses/level-2-lead/${prevModule.id}`}
        ...
        {nextModule && quizState === 'passed' ? (
          <Link
            to={`/courses/level-2-lead/${nextModule.id}`}
