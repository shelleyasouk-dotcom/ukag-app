...
import { SAFEGUARDING_COURSE } from '../../data/safeguardingCourse'
...
export function SafeguardingModulePage() {
  const { moduleId } = useParams<{ moduleId: string }>()
  const { profile } = useAuth()
  const mod = SAFEGUARDING_COURSE.modules.find(m => m.id === moduleId)
  ...
  if (!mod) return <Navigate to="/courses/safeguarding" replace />

  const moduleIndex = SAFEGUARDING_COURSE.modules.findIndex(m => m.id === moduleId)
  const prevModule = moduleIndex > 0 ? SAFEGUARDING_COURSE.modules[moduleIndex - 1] : null
  const nextModule = moduleIndex < SAFEGUARDING_COURSE.modules.length - 1 ? SAFEGUARDING_COURSE.modules[moduleIndex + 1] : null
  const isLastModule = moduleIndex === SAFEGUARDING_COURSE.modules.length - 1
  ...
  async function submitQuiz() {
    ...
    if (passed) {
      ...
      await supabase.from('course_progress').upsert({
        user_id: profile.id,
        course_id: SAFEGUARDING_COURSE.id,
        module_id: mod.id,
      ...
      if (isLastModule) {
        const { data: existing } = await supabase
          ...
          .eq('course_id', SAFEGUARDING_COURSE.id)
          .maybeSingle()
  ...
  return (
    <Layout>
      <div className="mb-2">
        <Link to="/courses/safeguarding" className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 mb-4">
          <ArrowLeft size={14} />
          Back to course
        </Link>
      </div>
      ...
      {/* Module header */}
      <div className="bg-gradient-to-br from-emerald-700 to-teal-800 rounded-xl p-6 mb-6 flex items-center gap-4">
        <span className="text-4xl">{mod.emoji}</span>
        <div>
          <div className="text-xs font-bold text-white/60 uppercase tracking-wide mb-0.5">
            Module {mod.number} of {SAFEGUARDING_COURSE.modules.length}
          </div>
      ...
      {tab === 'content' && (
        <div className="space-y-6">
                ...
                <ul className="space-y-2">
                  {section.bullets.map((b, bi) => (
                    <li key={bi} className="flex items-start gap-2 text-sm text-gray-700">
                      <span className="w-1.5 h-1.5 rounded-full flex-shrink-0 mt-2 bg-emerald-600" />
                      {b}
                    </li>
                  ))}
                </ul>
          ...
          <div className="flex justify-end">
            <button
              onClick={startQuiz}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold text-white bg-emerald-700 hover:bg-emerald-800 transition-colors"
              style={{ fontFamily: 'Montserrat, sans-serif' }}
            >
              Take the Quiz
              <ArrowRight size={16} />
        ...
        <div>
          {quizState === 'passed' && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center mb-6">
              ...
              {isLastModule && (
                <p className="text-xs text-green-600 mt-1">Your UKAG Safeguarding Certificate has been awarded and saved to your profile.</p>
              )}
            </div>
          )}
              ...
              return (
                <div key={qi} className="bg-white rounded-xl border border-gray-200 p-5">
                  <div className="flex items-start gap-3 mb-4">
                    <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 text-white bg-emerald-700" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                      {qi + 1}
                    </span>
                  ...
                  <div className="space-y-2">
                    {q.options.map((opt, ai) => {
                      ...
                      if (chosen === ai && !isRevealed) style = 'border-emerald-500 bg-emerald-50 text-emerald-800'
          ...
          {quizState === 'idle' && (
            <div className="mt-6 flex justify-end">
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold text-white bg-emerald-700 hover:bg-emerald-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                style={{ fontFamily: 'Montserrat, sans-serif' }}
              >
                Submit Answers
              </button>
      ...
      <div className="mt-8 flex justify-between gap-4">
        {prevModule ? (
          <Link
            to={`/courses/safeguarding/${prevModule.id}`}
        ...
        {nextModule ? (
          <Link
            to={`/courses/safeguarding/${nextModule.id}`}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold text-white bg-emerald-700 hover:bg-emerald-800 transition-colors"
            style={{ fontFamily: 'Montserrat, sans-serif' }}
            onClick={resetForNextModule}
          >
            {nextModule.title}
