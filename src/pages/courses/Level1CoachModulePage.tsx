...
export function Level1CoachModulePage() {
  const { moduleId } = useParams<{ moduleId: string }>()
  const { profile } = useAuth()
  const mod = LEVEL1_COACH_COURSE.modules.find(m => m.id === moduleId)
  ...
  if (!mod) return <Navigate to="/courses/level-1-assistant" replace />

  const moduleIndex = LEVEL1_COACH_COURSE.modules.findIndex(m => m.id === moduleId)
  const prevModule = moduleIndex > 0 ? LEVEL1_COACH_COURSE.modules[moduleIndex - 1] : null
  const nextModule = moduleIndex < LEVEL1_COACH_COURSE.modules.length - 1 ? LEVEL1_COACH_COURSE.modules[moduleIndex + 1] : null
  const isLastModule = moduleIndex === LEVEL1_COACH_COURSE.modules.length - 1
  ...
  return (
    <Layout>
      ...
      <div className={`bg-gradient-to-br ${mod.gradient} rounded-xl p-6 mb-6 flex items-center gap-4`}>
        ...
        <div>
          <div className="text-xs font-bold text-white/60 uppercase tracking-wide mb-0.5">
            Module {mod.number} of {LEVEL1_COACH_COURSE.modules.length}
          </div>
                ...
                <div key={qi} className="bg-white rounded-xl border border-gray-200 p-5">
                  ...
                  <div className="space-y-2">
                    {q.options.map((opt, ai) => {
                      ...
                      if (chosen === ai && !isRevealed) style = `border-[#ef462c] bg-[#ef462c]/10 text-[#ef462c]`
      ...
      <div className="mt-8 flex justify-between gap-4">
        {prevModule ? (
          <Link
            to={`/courses/level-1-assistant/${prevModule.id}`}
        ...
        {nextModule && quizState === 'passed' ? (
          <Link
            to={`/courses/level-1-assistant/${nextModule.id}`}
