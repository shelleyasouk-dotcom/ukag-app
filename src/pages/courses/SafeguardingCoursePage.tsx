...
export function SafeguardingCoursePage() {
  ...
  const course = SAFEGUARDING_COURSE
  ...
  return (
    <Layout>
      ...
      {/* Header */}
      <div className="bg-gradient-to-br from-emerald-700 to-teal-800 text-white rounded-xl px-5 pt-5 pb-6 mb-6">
        <div className="flex items-start justify-between gap-4 mb-3">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-widest text-emerald-200 mb-1">CPD — Safety Academy</p>
            <h1 className="text-2xl font-black leading-tight" style={{ fontFamily: 'Montserrat, sans-serif' }}>{course.title}</h1>
            <p className="text-emerald-100 text-sm mt-1">{course.subtitle}</p>
          </div>
      ...
      <div className="flex flex-col gap-3 pb-10">
        ...
        {certificate ? (
          <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex items-center gap-3 mb-1">
            <Award size={28} className="text-emerald-600 shrink-0" />
            <div className="flex-1">
              <p className="font-black text-emerald-800 text-sm" style={{ fontFamily: 'Montserrat, sans-serif' }}>Certificate Earned</p>
              <p className="text-xs text-gray-500">
                Completed {new Date(certificate.completed_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
              </p>
              <p className="text-xs text-emerald-700 font-semibold mt-0.5">{course.certificateTitle}</p>
            </div>
            <CertificateDownload
        ...
        <EnrollmentGate courseId={SAFEGUARDING_COURSE.id} courseTitle={SAFEGUARDING_COURSE.title}>
          {/* Module list */}
        ...
        {loading ? (
          ...
          course.modules.map((mod, index) => {
            ...
            return (
              <button
                key={mod.id}
                onClick={() => unlocked && navigate(`/courses/safeguarding/${mod.id}`)}
                disabled={!unlocked}
              ...
              >
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0 bg-gradient-to-br from-emerald-700 to-teal-800">
