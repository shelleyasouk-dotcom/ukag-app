    ...
    <Layout>
      ...
      <div className="bg-gradient-to-br from-violet-700 to-purple-800 text-white rounded-xl px-5 pt-5 pb-6 mb-6">
        <div className="flex items-start justify-between gap-4 mb-3">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-widest text-violet-200 mb-1">Professional Development</p>
            <h1 className="text-2xl font-black leading-tight" style={{ fontFamily: 'Montserrat, sans-serif' }}>{course.title}</h1>
            <p className="text-violet-200 text-sm mt-1">{course.subtitle}</p>
          </div>
      ...
      <div className="flex flex-col gap-3 pb-10">
        ...
        {certificate ? (
          <div className="bg-violet-50 border border-violet-200 rounded-2xl p-4 flex items-center gap-3 mb-1">
            <Award size={28} className="text-violet-500 shrink-0" />
        ...
        <EnrollmentGate courseId={AREA_LEAD_COURSE.id} courseTitle={AREA_LEAD_COURSE.title}>
          {/* Module list */}
        ...
        {loading ? (
          ...
          course.modules.map((mod, index) => {
            ...
            return (
              <button
                key={mod.id}
                onClick={() => unlocked && navigate(`/courses/area-lead/${mod.id}`)}
                disabled={!unlocked}
