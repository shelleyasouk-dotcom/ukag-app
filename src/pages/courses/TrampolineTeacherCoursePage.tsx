...
export function TrampolineTeacherCoursePage() {
  ...
  const course = TRAMPOLINE_TEACHER_COURSE
  ...
  return (
    <Layout>
      ...
      {/* Header */}
      <div className="bg-gradient-to-br from-cyan-700 to-teal-800 text-white rounded-xl px-5 pt-5 pb-6 mb-6">
        <div className="flex items-start justify-between gap-4 mb-3">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-widest text-cyan-200 mb-1">CPD — Coach Academy</p>
            <h1 className="text-2xl font-black leading-tight" style={{ fontFamily: 'Montserrat, sans-serif' }}>{course.title}</h1>
            <p className="text-cyan-100 text-sm mt-1">{course.description}</p>
          </div>
          <span className="text-4xl shrink-0">🤸</span>
        </div>
        <div className="mt-4">
          <div className="flex justify-between text-xs text-cyan-200 mb-1.5">
      ...
      <div className="flex flex-col gap-3 pb-10">
        ...
        {certificate ? (
          <div className="bg-cyan-50 border border-cyan-200 rounded-2xl p-4 flex items-center gap-3 mb-1">
            <Award size={28} className="text-cyan-600 shrink-0" />
            <div className="flex-1">
              <p className="font-black text-cyan-800 text-sm" style={{ fontFamily: 'Montserrat, sans-serif' }}>Certificate Earned</p>
              <p className="text-xs text-gray-500">
                Completed {new Date(certificate.completed_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
              </p>
              <p className="text-xs text-cyan-700 font-semibold mt-0.5">UKAG Trampolining Teacher Certificate Level 1 & 2</p>
            </div>
            <CertificateDownload
        ...
        <EnrollmentGate courseId={TRAMPOLINE_TEACHER_COURSE.id} courseTitle={TRAMPOLINE_TEACHER_COURSE.title}>
          {/* Module list */}
        ...
        {loading ? (
          ...
          course.modules.map((mod, index) => {
            ...
            return (
              <button
                key={mod.id}
                onClick={() => unlocked && navigate(`/courses/trampoline-teacher/${mod.id}`)}
                disabled={!unlocked}
                ...
                } ${done ? 'border-cyan-200' : 'border-gray-100'}`}
              >
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0 bg-gradient-to-br from-cyan-700 to-teal-800">
                  {done ? <CheckCircle size={22} className="text-white" /> : <span className="text-white text-sm font-black">{index + 1}</span>}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-[10px] font-extrabold uppercase tracking-wider text-gray-400">Module {index + 1}</span>
                    {done && <span className="text-[10px] font-bold bg-cyan-100 text-cyan-700 px-1.5 py-0.5 rounded-full">Complete</span>}
                  </div>
